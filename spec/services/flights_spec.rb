...
RSpec.describe Flights do
  ...
  search_request = Flights.search({fly_from:'CUU', fly_to:'MEX', date_from:tomorrow, date_to:tomorrow})
  before(:each) do
  	stub_request(:get, "#{ENV['flights_api_url']}&fly_from=CUU&fly_to=MEX&date_from=#{tomorrow}&date_to=#{tomorrow}").
         with(
           headers: {
       	  'Accept'=>'*/*',
...
', headers: {})
	...
	it 'must return flight params url' do
 		expect(search_request[0]["fly_from"]).to eq("CUU") 
	end
	it 'return flights airlines  details' do
	 p search_request[0] 
		expect(search_request[0]["airlines"][0]["logo_sm"]).to eq("https://images.kiwi.com/airlines/32/Y4.png") 
		expect(search_request[0]["airlines"][0]["logo_md"]).to eq("https://images.kiwi.com/airlines/64/Y4.png")
		expect(search_request[0]["airlines"][0]["iata"]).to eq("Y4")  
		expect(search_request[0]["airlines"][0]["name"]).to eq("Volaris")  
	end
	it 'must get flights routes' do
    expect(search_request[0]["routes"][0]["fly_from"]).to eq("CUU") 
	end
	it 'must get flights routess airlines details' do
		expect(search_request[0]["routes"][0]["airline"]["logo_sm"]).to eq("https://images.kiwi.com/airlines/32/Y4.png") 
		expect(search_request[0]["routes"][0]["airline"]["logo_md"]).to eq("https://images.kiwi.com/airlines/64/Y4.png")
		expect(search_request[0]["routes"][0]["airline"]["iata"]).to eq("Y4")  
		expect(search_request[0]["routes"][0]["airline"]["name"]).to eq("Volaris")  
	end
end
